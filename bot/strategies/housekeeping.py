"""Housekeeping jobs — the legacy run_cycle() work, on wall-clock timers.

Everything the old 5-minute loop did besides strategy entries lives here,
split into independently-scheduled jobs that run on the slow lane (they can
never delay the risk hot path):

  orders_and_queue   60s    manage open limit orders, process trade queue
  positions_sweep    300s   on-chain positions: resolutions, guardrails with
                            the illiquid-escalation state machine, redemption
  gov_feeds          300s   government RSS announcements
  earnings_watch     300s   earnings release scraper fast path
  news_llm           1800s  news scan + LLM position analysis (was "every
                            6th cycle")
  market_scan        3600s  LLM market scan + deep value scan (was "every
                            12th cycle")

The code is ported from bot/main.py's run_cycle() with its fix history
(#18, #21, #25, #29, #39, #61) intact; only the scheduling changed.
"""
...
from ..execution import (check_circuit_breakers, execute_buy, execute_sell,
                         get_usdc_balance, load_open_orders, log_trade,
                         get_stale_orders, order_succeeded, remove_order,
                         track_order)
...
def _is_strategy_held(token_id: str) -> bool:
    """True if a strategy plugin owns this position (hold-to-resolution)."""
    try:
        from ..core.strategy import is_strategy_held
        if is_strategy_held(token_id):
            return True
    except Exception:
        pass
    try:
...
def _effective_dry_run(dry_run: bool) -> bool:
    """Circuit breakers flip jobs to dry-run, exactly like the old loop."""
    if dry_run:
        return True
...
# ---------------------------------------------------------------------------
# Limit-order management + trade queue
# ---------------------------------------------------------------------------
...
def manage_open_orders(dry_run=False):
    ...
    try:
        ...
        return  # Can't check, skip this run
...
def job_orders_and_queue(ctx):
    """Open-order management + trade request queue."""
...
# ---------------------------------------------------------------------------
# Positions sweep: resolutions, guardrails, illiquid escalation, redemption
# ---------------------------------------------------------------------------
...
def job_positions_sweep(ctx):
    """On-chain position management — the old run_cycle steps 1–2c.

    The hot-path risk engine reacts to book EVENTS within milliseconds; this
    sweep is the belt-and-braces REST pass that also handles what events
    can't: resolutions, post-mortems, the illiquid-escalation state machine,
    and redemption.
    """
    ...
    raw = get_positions()
    portfolio = Portfolio.from_api(raw) if raw else None
    ...
    if portfolio:
        ...
        portfolio.positions = [p for p in portfolio.positions
                               if p.slug not in _already_resolved]
        resolved_slugs = []
        ...
        for pos in portfolio.positions:
            ...
            # Strategy positions (e.g. Tipoff 90) are hold-to-resolution:
            # interim dips are expected and must NOT trigger SL/TP.
            if _is_strategy_held(pos.token_id):
                continue

            gr = check_position(pos)
            if not gr.action.startswith("SELL"):
                continue
            ...
            if bid_price < config.MIN_SELL_PRICE or bid_depth < config.MIN_BID_DEPTH_USD:
                # No market liquidity — ALWAYS track the failure and check
                # escalation (must run even under circuit breaker — fix #18)
                ...
                if escalate and not is_escalated(pos.token_id):
                    # Force-sell is allowed even during circuit breaker — it
                    # reduces exposure rather than increasing it.
                    if bid_price > 0 and bid_depth > 0:
                        ...
                        # Only mark escalated on success (fix #21)
                        if result.get("success"):
                            mark_escalated(pos.token_id, "force_sell")
                        else:
                            log(f"  ⚠️ Force sell failed for {pos.title[:40]}, will retry next sweep")
                    else:
                    ...
                    if esc_action == "force_sell":
                        ...
                        # be force-sold again next sweep (fix #21).
            ...
            if not dry_run:
                execute_sell(pos.token_id, pos.size, pos.title,
...
def job_gov_feeds(ctx):
    """Government feed monitoring (feeds update infrequently)."""
    try:
        from ..api import get_positions
        from ..gov_monitor import check_gov_feeds, match_to_markets
        from ..portfolio import Portfolio
        gov_announcements = check_gov_feeds()
        if not gov_announcements:
            return
        positions = Portfolio.from_api(get_positions() or []).positions
        position_markets = [{"question": p.title, "title": p.title}
                            for p in positions]
...
def job_earnings_watch(ctx):
    """Earnings release scraper fast path."""
    try:
        from ..earnings_scraper import (get_watched_tickers,
                                        process_earnings_for_execution,
                                        execute_earnings_signal)
...
# ---------------------------------------------------------------------------
# Slow analysis: news + LLM position review, LLM market scan, deep value
# ---------------------------------------------------------------------------

def job_news_llm(ctx):
    """News scan + LLM position analysis (was: every 6th cycle)."""
    from ..api import get_positions
    from ..portfolio import Portfolio
    ...
    try:
        ...
        for pos in portfolio.positions:
            if _is_dust_position(pos):
                continue
            # Strategy positions are hold-to-resolution — never LLM-sell
            # them mid-flight.
            if _is_strategy_held(pos.token_id):
                continue

            # Noise reduction: skip LLM if price hasn't moved >5%
...
def job_market_scan(ctx):
    """LLM market scan + deep value scan (was: every 12th cycle)."""
    dry_run = _effective_dry_run(ctx.dry_run)
            ...
            if not analysis.strip():
                ...
                for line in analysis.split("\n"):
                    ...
                    try:
                        ...
                        # LEAN: auto-trade if research confirms (fix #61)
                        if is_lean:
                            if verdict == "TRADE":
...
# ---------------------------------------------------------------------------
# Registration + one-shot mode
# ---------------------------------------------------------------------------

def build_housekeeping_timers() -> list:
    """All housekeeping jobs as engine timers (slow lane)."""
    return [
        TimerSpec("housekeeping.orders_queue", 60, lane="slow",
                  run_at_start=True, fn=job_orders_and_queue),
        TimerSpec("housekeeping.positions_sweep", config.LOOP_INTERVAL_SEC,
                  lane="slow", run_at_start=True, fn=job_positions_sweep),
        TimerSpec("housekeeping.gov_feeds", config.LOOP_INTERVAL_SEC,
                  lane="slow", fn=job_gov_feeds),
        TimerSpec("housekeeping.earnings", config.LOOP_INTERVAL_SEC,
                  lane="slow", fn=job_earnings_watch),
        TimerSpec("housekeeping.news_llm", config.SCAN_INTERVAL_SEC,
                  lane="slow", fn=job_news_llm),
        TimerSpec("housekeeping.market_scan", config.DEEP_SCAN_INTERVAL_SEC,
                  lane="slow", fn=job_market_scan),
    ]


def run_once(dry_run: bool = False) -> dict:
    """One synchronous pass of the fast housekeeping + strategy checks —
    the old `--once` smoke run (no LLM scans, no WebSocket)."""
    from ..core.strategy import Context
    from ..core.books import BookCache

    ctx = Context(BookCache(), dry_run=dry_run)
    results = {}

    try:
        from ..threshold_monitor import run_threshold_check
        results["threshold"] = run_threshold_check(dry_run=dry_run)
    except Exception as e:
        log(f"⚠️ Threshold check: {e}")

    try:
        from ..tipoff90 import run_tipoff90_check
        results["tipoff90"] = run_tipoff90_check(dry_run=dry_run)
    except Exception as e:
    ...
    for job in (job_orders_and_queue, job_positions_sweep,
                job_gov_feeds, job_earnings_watch):
        try:
            job(ctx)
        except Exception as e:
            log(f"⚠️ {job.__name__}: {e}")
